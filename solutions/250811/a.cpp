...
private:
    ...
    // 区间查询：求区间 [l, r] 的和
    long long query_range(int node, int start, int end, int l, int r,long long c) {
        if (r < start || end < l) return 0;  // 区间无交集
        if (l <= start && end <= r) return tree[node]%(c+1);  // 当前区间完全包含在查询区间内
        ...
        long long left_sum = query_range(left_node, start, mid, l, r,c);
        long long right_sum = query_range(right_node, mid + 1, end, l, r,c);
        return (left_sum + right_sum)%(c+1);
    }
...
public:
    ...
    long long query(int l, int r, long long c) {
        return query_range(0, 0, n - 1, l, r , c);
    }
};
